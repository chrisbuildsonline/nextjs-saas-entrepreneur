...
const getAlertClasses = (variant?: "default" | "destructive") => {
  const baseClasses =
    "relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground"

  if (variant === "destructive") {
    return `${baseClasses} border-destructive/50 text-destructive dark:border-destructive [&>svg]:text-destructive`
  }
